...
func BuildProtocol(commandPrefix string, options *statesync.SyncOptions) string {
	...
	lines := []string{
		...
		"### Important task gate",
		"",
		"Optional gate behind manifest `tddmaster.importantTaskGate`. When enabled, any task flagged `important` pauses before execution for a plan-first review handled by the read-only `tddmaster-planner` subagent.",
		"",
		"**Marking tasks.** Users can bulk-flag tasks via a multiSelect prompt during SPEC_APPROVED (when the gate is on), or you can run:",
		"",
		"    " + commandPrefix + " spec <name> task mark-important <id>",
		"    " + commandPrefix + " spec <name> task unmark-important <id>",
		"",
		"**Gate payload.** When the next task is important and has no approved plan, `next` returns an `importantTaskGate` block with fields:",
		"",
		"- `taskId`, `phase` (`planning` | `review`), `attemptCount`, `priorFeedback`",
		"- `delegateAgent: \"tddmaster-planner\"`",
		"- `planSchema.fields`: `assumptions`, `touchedFiles`, `designPatterns`, `bestPractices`, `approach`",
		"- `userReviewOptions`: `[accept, revise, reject]`",
		"",
		"**Required orchestration.**",
		"",
		"1. Spawn `tddmaster-planner` (Read/Grep/Glob/LS only — never let it edit). Pass task scope plus `priorFeedback` when present.",
		"2. Receive its plan JSON. Present it to the user via AskUserQuestion with `accept | revise | reject`.",
		"3. Submit the result:",
		"   - accept → `" + commandPrefix + ` spec <name> next --answer='{"plan":{...},"accepted":true}'` + "`",
		"   - revise/reject → `" + commandPrefix + ` spec <name> next --answer='{"planFeedback":"<user's reason>"}'` + "` (gate re-fires; planner must address every prior-feedback point on retry)",
		"",
		"**Persistence + downstream injection.** An accepted plan is written to `progress.json` (`taskPlans[]`) and embedded as `approvedPlan` in every subsequent executor spawn for that task. `touchedFiles` is binding — if the executor would touch a file outside that list, STOP and report rather than expanding scope silently.",
		"",
		"**Toggling.** The gate is off by default. Outside `init`/`sync` it can be toggled with:",
		"",
	...
	lines = append(lines,
		...
		"In `DISCOVERY_REFINEMENT`, render `discoveryReviewData.reviewSummary` first.",
		"If `reviewSummary` is absent, render every `discoveryReviewData.answers` item before approval choices.",
		"Do NOT show approve/revise/split options until after the answer list is visible to the user.",
		"",
