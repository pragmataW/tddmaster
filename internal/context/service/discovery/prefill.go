...
func ExtractUserContextPrefills(raw string) []state.DiscoveryPrefillQuestion {
	...
	if utf8.RuneCountInString(trimmed) <= model.RichUserContextThreshold {
		return nil
	}

	segments := splitPrefillSegments(trimmed)
	results := make([]state.DiscoveryPrefillQuestion, 0, len(model.Questions))
...
func splitPrefillSegments(raw string) []string {
	...
	for _, part := range parts {
		...
		if utf8.RuneCountInString(segment) < model.PrefillSegmentMinRunes {
			continue
		}
...
func clipPrefillBasis(text string) string {
	...
	if len(runes) <= model.PrefillBasisMaxRunes {
		return string(runes)
	}
	return string(runes[:model.PrefillBasisMaxRunes-3]) + "..."
}
