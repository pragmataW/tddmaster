...
func init() {
	instructionMap[KeySettings] = "Before discovery, configure this spec's settings. Ask the user via the AskUserQuestion tool (multiSelect) which features to enable — never as plain prose. Present the four settings from interactiveOptions with their defaults: TDD/Red-Green-Refactor (default ON), Skip verifier (default OFF), Important task gate (default OFF), Min test coverage (default 80; 0 = disabled). A selected toggle means enabled (true); an unselected one means disabled (false). For min test coverage, ask the user for the percentage (0-100) and include it as the number \"minTestCoverage\". Aggregate the selections client-side and submit ALL four as a single JSON object: {\"tddEnabled\":true,\"skipVerifierEnabled\":false,\"importantTaskGateEnabled\":false,\"minTestCoverage\":80}."
